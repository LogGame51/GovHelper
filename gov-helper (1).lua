...
local mc, sc, wc = '{C6D13F}', '{C3AE22}', '{FFFFFF}'
local mcx = 0xD1B33F

local u8 = encoding.UTF8
encoding.default = 'CP1251'
local noErrorDialog = false
local tag = string.format("[ GOVERNMENT HELPER ] %s", wc)
local temp_prem = nil
local lections = {}
local status_button_gov = false
local go_credit = false
local unform_pickup_pos = { 1497.1835, -1280.1448, 113.8064 }
local stick_pickup_pos = { 1492.8258, -1277.4304, 113.8064 }
local jsn_upd = "https://gitlab.com/-/snippets/2384370/raw"
local lect_path = getWorkingDirectory() .. '\\GHelper\\Lections.json'
...
local cfg = inicfg.load({
	...
	govstr = {
		'[Правительство] Уважаемые жители штата!',
		'[Правительство] На данный момент проходит День Открытых Дверей в Правительство!',
		'[Правительство] Вам нужно явиться в Холл Мэрии! Ждём вас!'
	},
	govdep = {
		'[Пра-во] - [Всем] Занимаю гос.волну, просьба не перебивать!',
		'[Пра-во] - [Всем] Освобождаю гос.волну'
	},
	blacklist = {},
	Binds_Name = {},
	Binds_Action = {},
...
function main()
	...
	if not doesFileExist('moonloader/config/Gov_Config.ini') then
		if inicfg.save(cfg, 'Gov_Config.ini') then log('Создан файл Gov_Config.ini', "Подготовка") end
	end
	...
	addNotify("Меню: {90A5FF}/pravo (или Ctrl + B)\nВзаимодействие: {90A5FF}ПКМ + Q", 5)
	addNotify("Текущая версия: {90A5FF}"..thisScript().version.."\nСкрипт загружен!", 5)

	if cfg.main.infoupdate then
		infoupdate:switch()
		cfg.main.infoupdate = false
		inicfg.save(cfg, 'Gov_Config.ini')
	end

	sampRegisterChatCommand('pravo', function()
		bank:switch()
	end)

	if sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))) == 'Nikolaus_Heiliger' then
...
function se.onServerMessage(clr, msg)
	...
	local member, client, reason = msg:match('%[i%] (.+){FFFFFF} выгнал (.+) из мэрии! Причина: (.+)')
	if member and client and reason then
		if chat['expel'].v then
			msg = string.format(tag .. 'Сотрудник %s выгнал из мэрии %s по причине: %s', sc .. member .. wc, sc .. client .. wc, sc .. reason)
			return { 0x3F68D1FF, msg }
		end
		return false
	end

	local sum, nick = msg:match('^%[Пра-во%] {%x+}Организация получила (%d+%$) %(16 процентов%) за оплату штрафа игроком ([A-z0-9_]+)')
	...
	if nick and sum then
		if chat['incazna'].v then
			msg = string.format(tag .. 'Сотрудник %s пополнил счёт организации на %s', sc .. nick:gsub('_', ' ') .. wc, sc .. sum .. wc)
			return { 0x3F68D1FF, msg }
		end
		return false
	end
		...
		if msg:find('%[Объявление%]') then
			...
			if nick == "Jeffy_Cosmo" then
				...
				if msg:find("(( @bh_ver ))", 1, true) and not devmode then
					lua_thread.create(function()
						local text = ("/rb Gov-Helper %s"):format(thisScript().version)
...
function se.onShowDialog(dialogId, style, title, button1, button2, text) -- хук диалогов
	...
	if dialogId == 713 and isAnyAwaitExist then
		if await['pass_send'] then 		send(1, 0); await['pass_send'] = os.clock()
		elseif await['pass_date'] then 			send(1, 1); await['pass_date'] = nil
		elseif await['give_visa'] then 		send(1, 2); await['give_visa'] = nil
		end
		return false
	end

	if dialogId == 235 and await['get_rank'] then
		if not text:find('Правительство') and not devmode then
			lua_thread.create(function()
				wait(2000)
				addBankMessage('Вы не в организации {M}«Правительство»')
				addBankMessage('Скрипт работает только в этой организации')
				addBankMessage('Если вы считаете, что это ошибка - помолитесь')
				addBankMessage('Скрипт отключен..')
				log('Скрипт отключен. Вы не в организации "Правительство"')
				unload(false)
			end)
			return false
		end
	...
	if string.find(title, "Успеваемость") then
		...
		elseif not mMenu.status and mMenu.timer and os.clock() - mMenu.timer < 5.00 then
			...
			local cards = string.match(text, "Выдано ВИЗ: {FFB323}(%d+)")
			if cards then mMenu.cards.all = tonumber(cards) end
			local cards = string.match(text, "Выдано ВИЗ: {F9FF23}(%d+)")
			if cards then mMenu.cards.today = tonumber(cards) end

			local dep = string.match(text, "Выдано паспортов: {FFB323}(%d+)")
			if dep then mMenu.dep.all = tonumber(dep) end
			local dep = string.match(text, "Выдано паспортов: {F9FF23}(%d+)")
			if dep then mMenu.dep.today = tonumber(dep) end

			local vip = string.match(text, "Одобрено планировок: {FFB323}(%d+)")
			if vip then mMenu.vip.all = tonumber(vip) end
			local dep = string.match(text, "Одобрено планировок: {F9FF23}(%d+)")
...
function imgui.OnDrawFrame()
			...
			if TypeAction.v == 2 or TypeAction.v == 3 then
				...
				if TypeAction.v == 1 then
					if not go_credit then
						...
						if cfg.main.rank >= 2 then
							...
							imgui.Hint('expel11rank', u8'Доступно со X ранга')
						end
						...
						if cfg.main.rank >= 3 then
							if imgui.Button(u8('Выдать паспорт ')..fa.ICON_FA_ADDRESS_BOOK, imgui.ImVec2(250, 30)) then
								int_bank:switch()
								await['dep_minus'] = os.clock()
								play_message(MsgDelay.v, false, {
									{ "Здравствуйте, если вам нужно оформить паспорт ,представьтесь ,сколько вам лет и где проживаете?" },
									{ "/me {sex:открыл|открыла} тумбочку" }, --«»
									{ "/do Чистый паспорт в тумбочке." },
									{ "/me {sex:достал|достала} паспорт из тумбочки и {sex:записал|записала} всю информацию, передал паспорт %s", rpNick(actionId) },
									{ "/givepass %s", actionId }
								})
							end
							if imgui.IsItemClicked(1) then
								await['dep_minus'] = os.clock()
								sampSendChat(("/pravo %s"):format(actionId))
								int_bank:switch()
							end
						else
							imgui.DisableButton(u8('Выдать паспорт ')..fa.ICON_FA_LOCK, imgui.ImVec2(250, 30))
							imgui.Hint('depositdown5rank', u8'Доступно с 3 ранга')
						end

						imgui.SetCursorPos(imgui.ImVec2(165, 80))
						if cfg.main.rank >= 5 then
							if imgui.Button(u8('Оформление визы ')..fa.ICON_FA_ADDRESS_CARD, imgui.ImVec2(cfg.main.rank > 3 and 250 or -1, 30)) then
								int_bank:switch()
								await[cfg.main.rank >= 4 and 'dep_plus_10' or 'dep_plus'] = os.clock()
								play_message(MsgDelay.v, false, {
									{ "Здравствуйте, если вам нужно оформить визу ,представьтесь ,сколько вам лет и где проживаете.?" },
									{ "/me открыл тумбочку" },
									{ "/do Чистая виза тумбочке." },
									{ "/me достал визу из тумбочки и записал всю информацию, передал визу %s", rpNick(actionId) },
									{ "/givevisa %s", actionId }
								})
							end
							if imgui.IsItemClicked(1) then
								await[cfg.main.rank >= 5 and 'dep_plus_10' or 'dep_plus'] = os.clock()
								sampSendChat(("/pravo %s"):format(actionId))
								int_bank:switch()
							end
						else
							imgui.DisableButton(u8('Оформление визы ')..fa.ICON_FA_LOCK, imgui.ImVec2(-1, 30))
							imgui.Hint('depositeup5rank', u8'Доступно с 5 ранга')
						end

						if cfg.main.rank >= 11 then
							imgui.SameLine(nil, 5)
							...
							imgui.Hint('depositold', u8'Не работает')
							if imgui.IsItemClicked(1) then
								await['dep_plus'] = os.clock()
								sampSendChat(("/pravo %s"):format(actionId))
								int_bank:switch()
						...
						if cfg.main.rank >= 8 then
							if imgui.Button(u8('Поженить кого-то ')..fa.ICON_FA_HEART, imgui.ImVec2(-1, 30)) then
								go_credit = true
								play_message(MsgDelay.v, false, {
							...
							imgui.DisableButton(u8('Поженить кого-то ')..fa.ICON_FA_LOCK, imgui.ImVec2(-1, 30))
							imgui.Hint('credit8rank', u8'Доступно с 8 ранга')
						end

						imgui.SetCursorPos(imgui.ImVec2(165, 150))
						if cfg.main.rank >= 11 then
							if imgui.Button(u8('Не работает ')..fa.ICON_FA_MONEY_CHECK, imgui.ImVec2(291 / 2 - 2.5, 30)) then
								int_bank:switch()
								await['debt'] = os.clock()
								play_message(MsgDelay.v, false, {
							...
							imgui.DisableButton(u8('Не работает ')..fa.ICON_FA_LOCK, imgui.ImVec2(291 / 2 - 2.5, 30))
							imgui.Hint('debt11rank', u8'Доступно с X ранга')
						end

						imgui.SameLine(nil, 5)

						if cfg.main.rank >= 11 then
							if imgui.Button(u8('Не работает ')..fa.ICON_FA_PIGGY_BANK, imgui.ImVec2(291 / 2 - 2.5, 30)) then
								int_bank:switch()
							...
							imgui.DisableButton(u8('Не работает ')..fa.ICON_FA_LOCK, imgui.ImVec2(291 / 2 - 2.5, 30))
							imgui.Hint('howmuchmoney11rank', u8'Доступно с X ранга')
						end

						imgui.SetCursorPos(imgui.ImVec2(165, 185))
						if cfg.main.rank >= 11 then
							if imgui.Button(u8('Не работает ')..fa.ICON_FA_CREDIT_CARD, imgui.ImVec2(291 / 2 - 2.5, 30)) then
								int_bank:switch()
							...
							imgui.DisableButton(u8('Не работает ')..fa.ICON_FA_LOCK, imgui.ImVec2(291 / 2 - 2.5, 30))
							imgui.Hint('givecard11rank', u8'Доступно с X ранга')
						end

						imgui.SameLine(nil, 5)

						if cfg.main.rank >= 11 then
							if imgui.Button(u8('Не работает ')..fa.ICON_FA_TICKET_ALT, imgui.ImVec2(291 / 2 - 2.5, 30)) then
								int_bank:switch()
							...
							imgui.DisableButton(u8('Не работает ')..fa.ICON_FA_LOCK, imgui.ImVec2(291 / 2 - 2.5, 30))
							imgui.Hint('givevip11rank', u8'Доступно с Х ранга')
						end

						imgui.SetCursorPos(imgui.ImVec2(165, 220))
						if cfg.main.rank >= 11 then
							if imgui.Button(u8('Не работает ')..fa.ICON_FA_PLUS_CIRCLE, imgui.ImVec2(-1, 30)) then
								int_bank:switch()
							...
							imgui.DisableButton(u8('Не работает ')..fa.ICON_FA_LOCK, imgui.ImVec2(-1, 30))
							imgui.Hint('addcard11rank', u8'Доступно с Х ранга')
						end

						imgui.SetCursorPos(imgui.ImVec2(165, 255))
						if cfg.main.rank >= 11 then
							if imgui.Button(u8('Не работает ')..fa.ICON_FA_RECYCLE, imgui.ImVec2(-1, 30)) then
								imgui.OpenPopup(u8("Выбор действия##Card"))
							end
						else
							imgui.DisableButton(u8('Не работает ')..fa.ICON_FA_LOCK, imgui.ImVec2(-1, 30))
							imgui.Hint('restorecard11rank', u8'Доступно с Х ранга')
						end
						...
						if cfg.main.rank >= 11 then
							...
							imgui.Hint('getdeptime', u8'Не работает')
							if imgui.IsItemClicked(1) then
								await['dep_check'] = os.clock()
								sampSendChat(("/bankmenu %s"):format(actionId))
								int_bank:switch()
							...
							imgui.Hint('depositecheck11rank', u8'Доступно с Х ранга')
						end
					else
						...
						if imgui.Button(u8('Принять ')..fa.ICON_FA_USER_PLUS, imgui.ImVec2(norm_len / 2, 30)) then
							if not isPlayerOnBlacklist(sampGetPlayerNickname(actionId)) then
								...
								if cfg.main.rank >= 9 then
									...
									play_message(MsgDelay.v, true, {
										{ "Поздравляю! Вы нам подходите!" },
										{ "Сейчас я позову губернатора, что бы он Вас принял! Одну секундочку.." },
										{ 	(result
											and
											"/r Прошу подойти на пост №%s, нужно принять человека прошедшего собеседование")
										}
									})
								end
							else
				...
				if TypeAction.v == 3 then
					...
					if not mMenu.status then
						...
						imgui.TextColoredRGB( ("Выдано паспортов: " .. mc .. "%d"):format(mMenu.cards.all) )
						imgui.SameLine(180)
						imgui.TextDisabled(u8("(Сегодня: %d)"):format(mMenu.cards.today))

						imgui.TextColoredRGB( ("Выдано виз: " .. mc .. "%d"):format(mMenu.dep.all) )
						imgui.SameLine(180)
						imgui.TextDisabled(u8("(Сегодня: %d)"):format(mMenu.dep.today))

						imgui.TextColoredRGB( ("Одобрено интерьеров: " .. mc .. "%d"):format(mMenu.vip.all) )
						imgui.SameLine(180)
	...
	if bank.alpha > 0.00 then
		...
		imgui.BeginGroup()
			...
			imgui.TextColored(imgui.GetStyle().Colors[imgui.Col.ButtonHovered], fa.ICON_FA_GAVEL.. ' GOVERNMENT')
			local len_title = imgui.CalcTextSize(fa.ICON_FA_GAVEL.. ' GOVERNMENT').x
			imgui.PopFont()
		...
		if type_window.v == 3 then -- Правила
			...
			if imgui.Button(u8('Устав Правительства ')..fa.ICON_FA_BOOK_OPEN, imgui.ImVec2(-1, 35)) then
		...
		if type_window.v == 5 then -- Настройки
			...
			if imgui.Checkbox(u8'Автоматические обновления', loginupdate) then
				...
				if inicfg.save(cfg, 'Gov_Config.ini') then
			...
			imgui.BeginGroup()
				...
				if imgui.Button(u8('Ручное обновление ')..fa.ICON_FA_DOWNLOAD, imgui.ImVec2(150, 30)) then
					local url = "https://gitlab.com/i582/GovHelper/-/snippets/2384365/raw/main/1241212"
			...
			imgui.BeginGroup()
				if imgui.RedButton(u8'Удалить Gov-Helper', imgui.ImVec2(-1, 30)) then
				...
				if imgui.BeginPopupModal('##deleteBankHelper', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoTitleBar) then
					...
					imgui.CenterText(u8'УДАЛЕНИЕ GOV HELPER', imgui.ImVec4(0.8, 0.3, 0.3, 1.0))
					imgui.PopFont()
					imgui.Spacing()
					imgui.CenterText(u8'Вы действительно хотите удалить Gov-Helper?')
					...
					if imgui.RedButton(u8'Удалить', imgui.ImVec2(150, 30)) then
						...
						os.remove(getWorkingDirectory() .. '\\config\\Gov_Config.ini')
						unload(false)
					end
					imgui.SameLine()
			...
			imgui.NewLine()
			imgui.CenterTextColoredRGB('{AAAAAA}By ZOLOTAR, 2022')
			imgui.Spacing()
		end
	...
	if ustav_window.alpha > 0.00 then
		...
		imgui.Begin(u8'Устав Правительства', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar)
			...
			local results = 0
			for line in io.lines(getWorkingDirectory() .. "\\GHelper\\Устав ЦБ.txt") do
				if #line == 0 then line = "\n" end
			...
			if imgui.Button(fa.ICON_FA_PEN .. '##ustavedit', imgui.ImVec2(20, 20)) then
				os.execute('explorer '..getWorkingDirectory()..'\\GHelper\\Устав ЦБ.txt')
			end
			imgui.SameLine(nil, 5)
			...
			if v.active then
				...
				imgui.Begin(u8'##notifycard'..k, _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoFocusOnAppearing + imgui.WindowFlags.NoBringToFrontOnFocus)
					...
					imgui.BeginGroup()
						...
						imgui.PushFont(font[15])
						imgui.TextColoredRGB('{B4BEE1}Gov Helper')
						imgui.PopFont()
...
function go_expel(playerId, withoutRP)
	...
	if target_color ~= self_color then
		if not sampIsPlayerPaused(playerId) then
			...
			play_message(MsgDelay.v, true, {
				{ "Извините, но я вынужден выгнать вас из здания за неподобающее поведение." },
				{ "/me {sex:схватил|схватила} %s и {sex:повел|повела} к выходу", rpNick(playerId) },
				{ "/me {sex:открыл|открыла} дверь рукой и {sex:вывел|вывела} человека на улицу." },
				{ cmd_expel }
			})
		else
		...
		addBankMessage('Похоже, вы пытаетесь выгнать сотрудника правительства...')
	end
end
...
function system_cadr()
	if imgui.BeginPopupModal(u8("Кадровая система"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize) then
		...
		local cadrsys = io.open("moonloader/GHelper/Кадровая система.txt", "r+")
		...
		imgui.Hint('cardsystempath', u8('Нажмите что бы открыть файл:\n' .. getWorkingDirectory() .. '\\GHelper\\Кадровая система.txt'))
		if imgui.IsItemClicked() then
			os.execute('explorer '..getWorkingDirectory()..'\\GHelper\\Кадровая система.txt')
		end
		imgui.Spacing()
...
function system_uprank()
	if imgui.BeginPopupModal(u8("Единая система повышения"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize) then
		...
		local file = io.open("moonloader/GHelper/Система повышения.txt", "r+")
		...
		imgui.Separator()
		imgui.CenterTextColoredRGB('{464646}> Редактировать файл <')
		imgui.Hint('prosystematization', u8('Нажмите что бы открыть файл:\n' .. getWorkingDirectory() .. '\\GHelper\\Система повышения.txt'))
		if imgui.IsItemClicked() then
			os.execute('explorer '..getWorkingDirectory()..'\\GHelper\\Система повышения.txt')
		end
		imgui.Spacing()
		imgui.SetCursorPosX((imgui.GetWindowWidth() - 80) / 2)
...
function se.onSetSpawnInfo(team, skin, _, pos, rot, weapons, ammo)
	if CONNECTED_TO_ARIZONA then
		...
		if x == 1497 and y == -1283 and z == 113 then
...
function helpCommands()
	if imgui.BeginPopupModal(u8("Все команды скрипта"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar) then
		...
		imgui.BeginGroup()
			imgui.TextColoredRGB(mc..'/pravo{SSSSSS} - Основное меню скрипта (либо Ctrl + B)')
...
function checkData()
	local dir = getWorkingDirectory() .. '\\GHelper'
...
changelog = {
	...
	[2] = {
		...
		date = os.time({day = '07', month = '08', year = '2022'}),
		log = {
			"Исправил некоторые недочеты в скрипте",
			"Мой косяк тоже XD"
		},
		patches = {
			show = true,
	...
	[1] = {
		version = '1',
		comment = 'Главное обновление',
		date = os.time({day = '07', month = '08', year = '2022'}),
		log = {
			"Восстановил работоспособность скрипта"
		},
		patches = {
			show = true,
			info = {}
...
1.1: Устав Правительства - это внутриорганизационный регламентирующий деятельность сотрудников документ;
1.2: Устав может быть отредактирован только после того как большинство конгрессменов конгресса проголосуют"за" принятие изменений в нём;
...
1.5: Каждое нарушение устава карается наказанием начиная от устного выговора и заканчивая ЧС организации.

ОСТАЛЬНОЕ САМИ МЕНЯЙТЕ В ТЕКСТОВОМ ДОКУМЕНТЕ В ПАПКЕ С ХЕЛПЕРОМ!]]
